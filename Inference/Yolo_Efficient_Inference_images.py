...
def Yolo():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='yolov5-V5.0_best_weight_batch16_epoch20.pt',
                        help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='D:/NIH-git/poc1/Yolo_EfficientNet_/Inference/dataset/yolo', help='source')  # file/folder, 0 for webcam
...
def EfficientNet():
    ...
    parser.add_argument('--model_path', type=str, default="./weights/efficientnet_b4_fold1_best_accuracy.pt", metavar='S',
                        help='model path')
    parser.add_argument('--num_classes', type=int, default=4, metavar='N', help='num classes')
    parser.add_argument('--dataset_dir', type=str, default="D:/NIH-git/poc1/Yolo_EfficientNet_/Inference/dataset/effi",
                        metavar='S',
                        help='model path')
    opt = parser.parse_args()
    ...
    for idx, img_path in enumerate(image_list):
        ...
        cv2.putText(image1_copy, msg_fps, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        cv2.putText(image1_copy, msg_img, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
        cv2.putText(image1_copy, msg_class, (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

        effi_img_path = img_path[:-3]
        cv2.imwrite(effi_img_path + "-efficientnet.png", image1_copy)
        cv2.imshow("Efficientnet", image1_copy)
    ...
    csv_df.to_csv("./efficientnet_classification.csv", index=None)  # 수정; classify 결과 csv파일 이름
...
def Yolo_Efficient():
    ...
    parser.add_argument('--source', type=str, default='D:/NIH-git/poc1/Yolo_EfficientNet_/Inference/dataset/yolo_effi',help='source')  # file/folder, 0 for webcam
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.25, help='object confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='IOU threshold for NMS')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    ...
    parser.add_argument('--augment', action='store_true', help='augmented inference')
    parser.add_argument('--update', action='store_true', help='update all models')
    parser.add_argument('--project', default='D:/NIH-git/poc1/Yolo_EfficientNet_/Inference/dataset/yolo_effi', help='save results to project/name')
    parser.add_argument('--name', default='exp', help='save results to project/name')
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                # Write results
                im0_copy_box = im0.copy()
                im0_copy_text = im0.copy()
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        line = (cls, *xywh, conf) if opt.save_conf else (cls, *xywh)  # label format
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        img0_box = plot_one_box_only(xyxy, im0_copy_box, color=colors[int(cls)], line_thickness=3)
                        cv2.imwrite(name + "-efficientnet.png", img0_box)
                        #cv2.imshow("plot_one_box_only", img0_box)

                        im0_copy_text = plot_one_box(xyxy, im0_copy_text, label=label, color=colors[int(cls)], line_thickness=3)
                        cv2.imwrite(name + "-yolo.png", im0_copy_text)
                        #cv2.imshow("plot_one_box", im0_copy_text)
                        #cv2.waitKey(0)
            ...
            if save_img:
                if dataset.mode == 'image':
                    yolo_img_show = cv2.imread(name + "-yolo.png")
                    cv2.imshow("yolo", yolo_img_show)
        ...
        try:
            #crop 실패한 이미지 기록
            if os.path.isfile(data_image) == False:
                value = (data_image,"","","","","")
                csv_list.append(value)

            elif os.path.isfile(data_image):
                image_list.append(data_image)
            ...
            for idx, img_path in enumerate(image_list):
                ...
                yolo_img = name + "-efficientnet.png"
