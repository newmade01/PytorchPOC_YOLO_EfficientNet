...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Configure
    plots = not opt.evolve  # create plots: : label bounding box에 대한 정보를 시각화할지 정함
    cuda = device.type != 'cpu'
    init_seeds(2 + rank)    #seed random 초기화
    ...
    if pretrained:
        with torch_distributed_zero_first(rank):
            attempt_download(weights)  # 로컬에 값이 없으면 다운로드 받아옴
        ckpt = torch.load(weights, map_location=device)  # load checkpoint: epoch, best_fitness, training_result...etc
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create: 모델구조, 채널, 클래스갯수, anchors #yolo.py 함수안에 parse_model정의
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0, 픽셀 값을 0~1로 normalize
            ...
            # Forward
            with amp.autocast(enabled=cuda):    #tensor의 dtype이 자동으로 float16이나 float32로 바꿔줌=> 속도와 성능을 높이기 위해 도와줌
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=20) # 200이상은 필요
    ...
    parser.add_argument('--workers', type=int, default=8, help='maximum number of dataloader workers')  #데이터 로더의 갯 (램에 따라 조정: batch_size도 조정 필요)
    ...
    if opt.local_rank != -1:    #pc 1대 & gpu1대 => 둘다 -1
